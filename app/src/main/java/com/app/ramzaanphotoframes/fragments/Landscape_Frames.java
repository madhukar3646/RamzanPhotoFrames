    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK && requestCode == gallery_ReqCode) {
            ...
            if (resultCode == -1) {
                ...
                try {
                    Intent intent = new Intent(getActivity(), Editing_Activity_lpf.class);
                    intent.putExtra("imageUri", resultUri.toString());
                    intent.putExtra("selectedFramePosition", landscape_list.get(this.selectedFramePosition));
                    intent.putStringArrayListExtra("frames",landscape_list);
                    startActivity(intent);
                } catch (Exception e) {
                    e.printStackTrace();
                }
